...
function getHostEnvironment() {
  ...
  if (host.toLowerCase().indexOf("http://216.243.141.55/staging") == -1) { //production
    environment = "production";
  }
  else if (host.toLowerCase().indexOf("216.243.141.55/staging") != -1) { //staging
    environment = "staging";
  }
  return environment;
}
...
function createFormstackReferenceLink() {
  var host_env = getHostEnvironment();
  ...
  // Staging 
  if (host_env=="staging") {
    var formId = $jq("#Custom3").val();
    var uniqueId = $jq("#Custom1").val();
    var projectId = $jq('.box_title_big').text();
    var assignedToName = $jq('.request-assignedto-name').text();

    // The /3778453 is a proprietary number added at the end of a view link by Formstack per form. It is NOT equal to the FormID
    formstackViewLink = 'https://www.formstack.com/admin/submission/view/' + uniqueId + '/3778453?share=MZ2TmNlZN2';
    formstackEditLink = 'https://www.formstack.com/admin/submission/edit/' + uniqueId + '/S3778453';
    serviceAgreementEditLink = 'http://s.sc-ctsi.org/service_agreements/new/' + uniqueId + '/' + projectId + '/' + assignedToName;
